...
theApplication = "addferti-bursa-soil-moisture"
...
try:
  ...
  postgreSQLConnection.execute("DELETE FROM bursa_soil_moisture t WHERE EXISTS (SELECT FROM bursa_soil_moisture WHERE device_id = t.device_id AND recieved_at = t.recieved_at AND ctid < t.ctid order by recieved_at);")
except:
  print("except...")
